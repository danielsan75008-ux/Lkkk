    ...
    if on then
        for _, v in ipairs(char:GetDescendants()) do
            if v:IsA("BillboardGui") or v:IsA("TextLabel") then
                if v.Name == "NameTag" or v.Name == "Head" or
                   (v:IsA("BillboardGui") and v.Name == "HealthBar") then
                    continue
                end
                pcall(function()
...
do
    ...
    local deathLabel = TabServerInfo:Section({ Title = "☠️ Mortes: 0" })
    ...
    task.spawn(function()
        while task.wait(1) do
            pcall(function()
                ...
                if deathLabel then deathLabel.Title = "☠️ Mortes: " .. deathCount  end
            end)
        end
    end)
end
